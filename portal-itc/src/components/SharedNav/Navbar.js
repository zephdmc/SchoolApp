...
const Navbar = () => {
  ...
  return (
    <>
      <TopNav />
      <nav className={`fixed ${isVisible ? 'top-8' : 'top-2'} ${isVisible ? 'shadow-md' : 'shadow-[1px]'} left-0 w-full ${isVisible ? 'md:top-1 lg:top-8 top-[1px] bg-blue-300' : 'bg-current'} text-itccolor z-30 transition-transform duration-300 ${isVisible ? 'translate-y-0' : '-translate-y-full'}`}>
        <div className="mx-auto flex md:justify-start justify-between items-center md:px-[3%] lg:px-[10%] p-2">
          <div className="text-2xl ml-2 md:ml-[4%] flex lg:ml-[2%] text-left font-bold">
            <img src="images/AcanetLogo.png" alt="Acanet_Logo" className="h-[50px] w-[90px]" />
            <p className="md:text-md lg:text-md my-4 pr-12 text-itccolor md:block hidden"> School Managment</p>
          </div>
