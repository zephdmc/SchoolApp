...
import { getAllRoutes, deleteRoute, updateRouteProfile, AddRoute, getAllRouteByRegistered } from '../../services/routeService';
...
const ManageRoutes = () => {
    ...
    const fetchCarsAndRoutes = async () => {
      try {
        if (user) {
          ...
          const routeData = await getAllRouteByRegistered(user._id);
  ...
  const handleUpdateRoute = async (updateData) => {
    const completeRouteData = { ...updateData, registeredBy }; // ensure carData is an object
    try {
      await updateRouteProfile(currentRoute._id, completeRouteData);
      setRoutes(routes.map((route) => (route._id === currentRoute._id ? { ...route, ...updateData } : route)));
  ...
  const handleAddRoute = async (routeData) => {
    if (typeof routeData !== 'object') {
      console.error('Invalid carData format:', routeData);
      setError('Invalid car data. Please try again.');
      return;
    }
    const completeRouteData = { ...routeData, registeredBy }; // ensure carData is an object
    try {
      await AddRoute(completeRouteData);
      const updatedCars = await getAllRouteByRegistered(user._id);
      setRoutes(updatedCars);
  ...
  return (
    <div className="p-4">
      ...
      <div className="flex flex-col sm:flex-row sm:justify-between items-center mb-4">
        <h1 className="text-2xl font-bold mb-4 sm:mb-0">Routes Overview</h1>
      ...
      {isAddRouteModalOpen && (
        <div className="fixed inset-0 z-10 overflow-y-auto bg-black bg-opacity-50 flex items-center justify-center p-4">
          ...
          <div className='bg-itccolor p-4 mb-2 '>
              <h2 className="text-xl font-bold text-white rounded mb-2">Add Route</h2>
            ...
            >
              <div className="px-6">
                ...
                <div className="mb-4">
                  ...
                  <input type="number" name="distance" id="distance" className="mt-1 block w-full border border-gray-300 rounded-md p-2" required />
                </div>
                <div className="mb-4">
      ...
      {isViewRouteModalOpen && currentRoute && (
        <div className="fixed inset-0 z-10 overflow-y-auto bg-black bg-opacity-50 flex items-center justify-center p-4">
          <div className="w-full max-w-md bg-white rounded-lg shadow-xl">
          <div className='bg-itccolor p-4 mb-2 '>
              <h2 className="text-xl font-bold text-white rounded mb-2">View Route</h2>
      ...
      {isEditRouteModalOpen && currentRoute && (
        <div className="fixed inset-0 z-10 overflow-y-auto bg-black bg-opacity-50 flex items-center justify-center p-4">
          <div className="w-full max-w-md bg-white rounded-lg shadow-xl">
          <div className='bg-itccolor p-4 mb-2 '>
              <h2 className="text-xl font-bold text-white rounded mb-2">EDIT ROUTE</h2>
              </div>            <form
              onSubmit={(e) => {
                e.preventDefault();
                const updatedRoute = {
      ...
      {isDeleteRouteModalOpen && currentRoute && (
        <div className="fixed inset-0 z-10 overflow-y-auto bg-black bg-opacity-50 flex items-center justify-center p-4">
          <div className="w-full max-w-md bg-white rounded-lg shadow-xl">
            <h3 className="text-lg font-medium leading-6 text-gray-900 p-6">Delete Route</h3>
            <p className="px-6">Are you sure you want to delete this route?</p>
            <div className="px-6 py-4 bg-gray-50 flex justify-end space-x-4">
