...
const TeacherExamTimetable = () => {
  ...
  const [timetables, setTimetables] = useState([]); // Changed to array for multiple timetables
  const [loading, setLoading] = useState(true);
  const [selectedDate, setSelectedDate] = useState(new Date());
  const [activeTab, setActiveTab] = useState('list');
  const [academicYears, setAcademicYears] = useState([]);
  const [teacherName, setTeacherName] = useState(''); // Changed from className
  ...
  useEffect(() => {
    if ((selectedYear || academicYears.length > 0) && teacherName) {
      fetchTimetables();
    }
  }, [selectedYear, teacherName, selectedTerm]); // add selectedTerm
  ...
  const fetchTeacherProfile = async () => {
    try {
      console.log(user?._id, 'id teacherID')
        const res = await getTeacherByID(user?._id);
        console.log(res, 'response from TeacherID get')
  ...
  const fetchTimetables = async () => {
    try {
      setLoading(true);
      const res = await axios.get('/academic/api/examtimetable/teacher', {
        params: { 
          supervisorName: teacherName?.trim(),
          academicYear: selectedYear,
          term: selectedTerm || undefined, // send only if selected
        }
      });
      console.log(res, 'ete');
  ...
  // Combine all schedules from multiple timetables
  const allSchedules = timetables.flatMap(t => t.schedule || []);

  const filteredExams = allSchedules.filter(item => {
    try {
  ...
  return (
    <div className="container mx-auto px-4 py-8">
      <h1 className="text-2xl font-bold mb-6 text-gray-800">Exam Supervision Timetable</h1>
...
>
  ...
  {terms.map((term) => (
    <option key={term?._id || term?.name} value={term?.name}>
      {term?.name}
    </option>
  ))}
</select>
      ...
      {loading ? (
        ...
        <>
          <div className="bg-blue-600 text-white px-6 py-4 rounded-t-lg">
            <h2 className="text-xl font-semibold">Your Exam Supervision Schedule</h2>
            <p>Teacher: {teacherName || 'N/A'} | Academic Year: {selectedYear || 'N/A'}</p>
          </div>
                  ...
                  <div key={index} className="px-4 py-3 hover:bg-gray-50">
                    <div className="grid grid-cols-1 md:grid-cols-5 gap-3">
                      ...
                      <div className="text-sm">
                        Class: {timetables.find(t => t.schedule?.includes(item))?.className || 'N/A'}
                      </div>
                      <div className="text-sm">
                        Room: {item?.room || 'N/A'}
                      </div>
                  ...
                  {filteredExams.map((exam, index) => (
                    <div key={index} className="border-l-4 border-blue-500 pl-3 py-2 bg-gray-50 rounded">
                      ...
                      <div className="text-xs text-gray-500 mt-1">
                        ...
                        <span>Class: {timetables.find(t => t.schedule?.includes(exam))?.className || 'N/A'}</span>
                      </div>
                      {exam?.notes && (
        ...
        <div className="bg-white rounded-lg shadow-md p-6 text-center text-gray-500">
          No exam supervision assigned to {teacherName || 'you'} in {selectedYear || 'selected year'}
        </div>
      )}
    </div>
  );
